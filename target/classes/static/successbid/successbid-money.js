...
$("#autoCalculate").click(function () {
    ...
    var builderCost = $("input[name='builderCost']").val();
    var labourCost = $("input[name='labourCost']").val();
    var labourTotal = $("input[name='labourTotal']").val();
    var labourReceived = $("input[name='labourReceived']").val();
    var labourLack = $("input[name='labourLack']").val();
    ...
    //应付项目工程款计算
    if(isNotPirce(collectedAmount)||isNotPirce(managementCost)||isNotPirce(incomeTax)
        ||isNotPirce(insurancePremium)||isNotPirce(deductibleTax)||isNotPirce(addedTax)){
        alert("保存失败，金额必须是数字！！！");
        return;
    }else{
    $("input[name='projectPayment']").val(collectedAmount-managementCost-incomeTax-accountInvoice-insurancePremium-builderCost+deductibleTax/1
            +addedTax/1-temporaryPayment);//应付项目工程款 （10=2-3-4-5-6+7+8-9）
    }

    if(isNotPirce(labourTotal)||isNotPirce(labourReceived)){
            alert("保存失败，金额必须是数字！！！");
            return;
        }else{
            $("input[name='labourLack']").val(labourTotal-labourReceived);//尚缺劳务 （应收劳务-已收劳务）
        }
